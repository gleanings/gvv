...
- 存在同名 tag 时提示是否移除同名 tag 后重新构建

## 1.0.2 (2026-6-29)

- 修补设置 remote 时无法输入的问题
...
- 在没有设置默认分支且上游分支时，不再提示错误后退出，而是让用户自己选择要推送的分支

### 🐛 修复

- 修复错误退出时执行 `git restore --staged xxx` 命令后多了空字符串
- 修复 `fatal: not a valid object name: main` 错误（**未验证**）
...
- 修复已知问题，该问题导致在仅一个上游别名时获取别名为 `undefined`
